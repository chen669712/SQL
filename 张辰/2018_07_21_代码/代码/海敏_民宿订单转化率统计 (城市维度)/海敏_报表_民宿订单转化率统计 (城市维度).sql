...
------------------------------------报表(测试,城市)-------------------------------------------------------
SELECT 
    city.cityName as `城市名称`
  , a.visitNumber as `dau`
  , b.ois as `日支付订单数`
  , concat_ws('',cast(round((b.ois*100)/a.visitNumber, 2) as string),'%') as `日转化率` 
  , c.ois as `日民宿订单数`
  , d.ois as `日客栈订单数`
  , e.visitNumber as `wau`
  , f.ois as `周支付订单数`
  , concat_ws('',cast(round((f.ois*100)/e.visitNumber, 2) as string),'%') as `周转化率`
  , g.ois as `周民宿订单数`
  , h.ois as `周客栈订单数`
  , i.visitNumber as `mau`
  , j.ois as `月支付订单数`
  , concat_ws('',cast(round((j.ois*100)/i.visitNumber, 2) as string),'%') as `月转化率`
  , k.ois as `月民宿订单数`
  , l.ois as `月客栈订单数`
--使用搜索的用户数
from  
...
------------------------------------报表(测试,城市,bnb_hive_db.bnb_tracelog)-------------------------------------------------------
SELECT 
    city.cityName as `城市名称`
  , a.visitNumber as `dau`
  , b.ois as `日支付订单数`
...
inner join ods_htl_groupwormholedb.bnb_city city on a.cityname = city.cityname and city.d = '2018-07-18'       
inner join
--日客栈民宿订单  
(select t3.cityid
          , sum (t3.ois) as ois
