...
where substring(b1.createdtime,0,10) >='2018-06-22'
  and substring(b1.createdtime,0,10) <='2018-07-21'
  and a1.d='2018-07-22'
...
where to_date(orderdate) >='2018-06-22'
  and to_date(orderdate) <='2018-07-21'
...
left outer join ods_htl_groupwormholedb.bnb_city city on search_m.cityid = city.cityid and city.d = '2018-07-22'
group by city.cityname;
union all
select  '汇总' as cityname
  , count (distinct search_m.cid ) as MAU
  , count (distinct pay_m.orderid) as order_w
  , concat(round(count (distinct pay_m.orderid)/count (distinct search_m.cid )*100,2),'%') as conversion_m 
  , count (distinct bnbpay_m.orderid ) as bnborder_m
  , count (distinct hotelpay_m.orderid) as hotelorder_m
from
